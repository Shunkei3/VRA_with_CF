...
def batch_pred(X):
    ...
    for i in range(math.ceil(X.shape[0]/batch_size)):
        with torch.no_grad():
            ...
            x6 = AvrTrans(x6)
            x6 = torch.from_numpy(x6).float().view(-1,1)
            x6 = x6.to(device)
            
            pred = model(x0,x1,x2,x3,x4,x5,x6).detach().cpu().numpy().astype('float').tolist()
...
for simID in np.unique(df.sim):
    ...
    for i in set(data.subplot_id):
        for j in set(data.strip_id):
            ...
            X_array = np.stack([alpha1_temp,alpha2_temp,beta1_temp,beta2_temp,ymax1_temp,ymax2_temp,rate_temp],axis=2)
    ...
    for epoch in range(EPOCH):
        ...
        for phase in ['train', 'val']:
            ...
            for data in data_loaders[phase]:
                ...
                outputs = model(x0,x1,x2,x3,x4,x5,x6)
    ...
    for i in range(X_test.shape[0]):
        for value in range(int(np.min(data2.rate)), int(np.max(data2.rate)+1)):
            ...
            with torch.no_grad():
                ...
                x6 = np.array(value).astype('float')
                x6 = scaler_rate.transform([[x6]]) # Standardize #Please chekc!!!!
                x6 = torch.from_numpy(x6).float().view(-1,1)
                x6 = x6.to(device)

                pred = model(x0,x1,x2,x3,x4,x5,x6).detach().cpu().numpy().astype('float').reshape(-1).tolist()
            pred_list.extend(pred)

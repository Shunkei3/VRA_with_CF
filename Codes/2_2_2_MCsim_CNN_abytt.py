...
def batch_pred(X):
    ...
    for i in range(math.ceil(X.shape[0]/batch_size)):
        with torch.no_grad():
            ...
            x5 = AvrTrans(x5)
            x5 = torch.from_numpy(x5).float().view(-1,1)
            x5 = x5.to(device)
            
            pred = model(x0,x1,x2,x3,x4,x5).detach().cpu().numpy().astype('float').tolist()
...
for simID in simIDs:
    ...
    for i in set(data.subplot_id):
        for j in set(data.strip_id):
            ...
            X_array = np.stack([alpha_temp,beta_temp,ymax_temp,theta_1_temp,theta_2_temp,rate_temp],axis=2)
    ...
    for epoch in range(EPOCH):
        ...
        for phase in ['train', 'val']:
            ...
            for data in data_loaders[phase]:
                ...
                outputs = model(x0,x1,x2,x3,x4,x5)
    ...
    for i in range(X_test.shape[0]):
        for value in range(int(np.min(data2.rate)), int(np.max(data2.rate)+1)): 
            ...
            with torch.no_grad():
                ...
                x5 = np.array(value).astype('float')
                x5 = scaler_rate.transform([[x5]]) # Standardize #Please chekc!!!!
                x5 = torch.from_numpy(x5).float().view(-1,1)
                x5 = x5.to(device)

                pred = model(x0,x1,x2,x3,x4,x5).detach().cpu().numpy().astype('float').reshape(-1).tolist()
            pred_list.extend(pred)
